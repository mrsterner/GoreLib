...
public class ParticleUtils {
    ...
    public static void spawnItemParticleBeam(Vec3d from, Vec3d to, ServerWorld serverWorld, ItemStack itemStack) {
        ...
        double x = from.getX() + (serverWorld.random.nextDouble() * 0.2D) + 0.4D;
        double y = from.getY() + (serverWorld.random.nextDouble() * 0.2D) + 1.2D;
        double z = from.getZ() + (serverWorld.random.nextDouble() * 0.2D) + 0.4D;
        if (!itemStack.isEmpty()) {
            serverWorld.spawnParticles(
                    new ItemStackBeamParticleEffect(GoreLibParticleTypes.ITEM_BEAM_PARTICLE, itemStack, 10),
                    x,
                    y,
                    z,
                    0,
