    ...
    private void cleanup() {
        if (this.lifespan >= 0L) {
            ...
            if (time - this.lastCheck > this.lifespan) {
                Collection<K> collect = this.cache.entrySet().stream().filter((kValueWrapperEntry) -> (kValueWrapperEntry.getValue()).checkInvalid(time)).map(Map.Entry::getKey).collect(Collectors.toSet());
